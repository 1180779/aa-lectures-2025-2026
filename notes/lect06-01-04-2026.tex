...
\begin{dygresja}
	Pokażmy dokładniej, dlaczego $S_{ij} = \emptyset$ dla $i \geq j$.
	Przypuśćmy, że istnieje $z_k \in S_{ij}$. Z~definicji $S_{ij}$ mamy wtedy
	$t_i \leq s_k < t_k \leq s_j$, a~ponieważ zajęcia są posortowane i~$i \geq j$,
	to $t_j \leq t_i$, skąd $s_j < t_j \leq t_i$. Łącząc obie nierówności,
	otrzymujemy $s_j < t_i \leq s_k < t_k \leq s_j$, czyli $s_j < s_j$ -- sprzeczność.
\end{dygresja}
...
\begin{proof}~\\
	\textbf{Ad~\ref{lem:greedy-exists}.} Niech $A_{ij}$ będzie podzbiorem o największej liczności składającym się z parami zgodnych zajęć zbioru $S_{ij}$. Niech $z_k$ będzie zajęciem z $A_{ij}$ o najmniejszym czasie zakończenia.
	...
	\textbf{Ad~\ref{lem:greedy-empty}.} Przypuśćmy, że nie. Niech $z_k \in S_{im}$. Wtedy $t_i \leq s_k < t_k \leq s_m < t_m \leq s_j$. Zatem $z_k \in S_{ij}$ oraz $t_k < t_m$ -- sprzeczność z definicją zajęcia $z_m$.
\end{proof}
