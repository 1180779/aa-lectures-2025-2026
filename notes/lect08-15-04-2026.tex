...
\begin{flalign*}
	...
	\vphantom{\sum_{j=1}^n \left( \frac{n}{3} - \left\lfloor \frac{j-1}{3} \right\rfloor \right)} & = \frac{n}{3} (t_{i_1} + t_{i_2} + t_{i_3}) + \left(\frac{n}{3} - 1\right) (t_{i_4} + t_{i_5} + t_{i_6}) + \left(\frac{n}{3} - 2\right) (t_{i_7} + t_{i_8} + t_{i_9}) +                                                                      &  & \\
	\vphantom{\sum_{j=1}^n \left( \frac{n}{3} - \left\lfloor \frac{j-1}{3} \right\rfloor \right)} & \hphantom{=} {} + \dots + 1 \cdot (t_{i_{n-2}} + t_{i_{n-1}} + t_{i_n}) =                                                                                                                                                                    &  & \\
...
\begin{proof}
	Niech $\Pi = (i_1, \dots, i_n)$ będzie permutacją minimalizującą $f$. Przypuśćmy, że dla pewnych $k, l \in [n]$, $k < l$ ale $t_{i_k} > t_{i_l}$. Niech $\Pi' = (i'_1, i'_2, \dots, i'_n)$ będzie permutacją powstałą z $\Pi$ poprzez zamianę $i_k$ z $i_l$, tzn.
	\begin{flalign*}
		 &  & \begin{cases}
			      i'_k = i_l, \quad i'_l = i_k & \text{(zamieniane pozycje)}               \\
			      i'_j = i_j                   & \text{dla\ } j \in [n] \setminus \{k, l\}
		      \end{cases}\text{. Wtedy:} &  &  &  &  &  &
	\end{flalign*}
	\begin{flalign*}
		f(\Pi) - f(\Pi') & = \sum_{j=1}^n A_j \cdot t_{i_j} - \sum_{j=1}^n A_j \cdot t_{i'_j} =                                            &  & \\
		                 & = \vphantom{\sum_{j=1}^n} A_k \cdot t_{i_k} + A_l \cdot t_{i_l} - A_k \cdot t_{i'_k} - A_l \cdot t_{i'_l} =     &  & \\
		                 & = \vphantom{\sum_{j=1}^n} A_k \cdot t_{i_k} + A_l \cdot t_{i_l} - A_k \cdot t_{i_l} - A_l \cdot t_{i_k}  =      &  & \\
		                 & = \vphantom{\sum_{j=1}^n} A_k(t_{i_k} - t_{i_l}) - A_l(t_{i_k} - t_{i_l}) =                                     &  & \\
		                 & = \vphantom{\sum_{j=1}^n} \underbrace{(t_{i_k} - t_{i_l})}_{> 0} \cdot \underbrace{(A_k - A_l)}_{\geq 0} \geq 0 &  &
	\end{flalign*}
