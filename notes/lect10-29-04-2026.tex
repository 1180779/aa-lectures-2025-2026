...
\begin{obowiazkowy}
	\begin{theorem}
		Niech $S$ -- zbiór zadań o jednostkowym czasie wykonania, a $\mathcal{I}$ -- rodzina zbiorów wolnych od kar. Wtedy $(S, \mathcal{I})$ jest matroidem.
	\end{theorem}
	\begin{proof}
		...
		\begin{enumerate}[
				...
				labelwidth=\labelw,
				labelsep=0pt,
				leftmargin=\dimexpr\labelw-2em\relax,  % wrapped lines hang 1em LEFT of first-line text
				itemindent=2em
			]
			\item[(wł.\ \ref{matroid:skończoność})] Oczywiste. $\checkmark$
			\item[(wł.\ \ref{matroid:niepustość})] Dla każdego $i \in [n]$ zachodzi $\{z_i\} \in \mathcal{I}$, więc $\mathcal{I} \neq \emptyset$. $\checkmark$
			\item[(wł.\ \ref{matroid:dziedziczność})] Niech $A \in \mathcal{I}$ (czyli $A$ jest wolny od kar) i $B \subseteq A$. Z~definicji istnieje harmonogram zbioru $A$, w którym wszystkie zadania są terminowe. Usuwając z~tego harmonogramu zadania należące do $A \setminus B$, otrzymujemy harmonogram zbioru $B$, w którym wszystkie zadania nadal są terminowe (usunięcie zadań nie opóźnia pozostałych). Zatem $B$ jest wolny od kar, czyli $B \in \mathcal{I}$.
			\item[(wł.\ \ref{matroid:wymiana})] Niech $A, B \in \mathcal{I}$, $|B| > |A|$.\\
		...
		\end{enumerate}
		Pokazaliśmy, że $(S, \mathcal{I})$ jest matroidem.
	\end{proof}
\end{obowiazkowy}

Możemy do rozwiązania problemu szeregowania zadań użyć algorytmu Greedy (Algorytm \ref{alg:greedy}) dla matroidu $(S, \mathcal{I})$ maksymalizującego $\sum_{z_i \in A} w_i$ (czyli minimalizującego sumę kar zadań spóźnionych).

Z twierdzenia~\ref{thm:rado-edmonds} (Rado, Edmonds) algorytm Greedy użyty do tego problemu zwraca rozwiązanie optymalne. Warunek z linii~\ref{line:greedy-niezaleznosc} algorytmu Greedy (tzn. czy $A \cup \{x_i\} \in \mathcal{I}$) można sprawdzić w czasie $\BO(n)$ -- lemat \ref{lemma:wolny-od-kar}, warunek~\ref{lemma:wolny-od-kar:sort}.
