...
\begin{obowiazkowy}
	...
	\begin{itemize}[align=left, widest={\uemph{Zwyciężaj:}}, leftmargin=!, labelsep=0.5em]
		\item[\uemph{Dziel:}] Dzielimy punkty $P$ pionową prostą $l$ na dwa podzbiory $P_L$ i $P_R$ na lewo i na prawo
		      ...
		      \begin{remark}
			      Prostą $l$ definiuje \emph{mediana współrzędnej~$x$} zbioru~$P$
			      --- tak ją zadają Preparata i~Shamos (\textit{Computational
			      Geometry: An Introduction}, \S5.4), za~którymi podąża też CLRS
			      (\S33.4). Dla parzystego $|P|$ medianę bierzemy jako średnią
			      dwóch środkowych wartości, $x_l = \frac{1}{2}\!\left(X\!\left(\tfrac{|P|}{2}\right).x
			      + X\!\left(\tfrac{|P|}{2}+1\right).x\right)$, co daje
			      $|P_L| = |P_R| = \tfrac{|P|}{2}$. Sam \emph{algorytm} nie wymaga
			      jednak żadnej wyróżnionej wartości: istotny jest tylko podział
			      na~$\lceil|P|/2\rceil$ i~$\lfloor|P|/2\rfloor$ punktów (dowolna prosta
			      rozdzielająca posortowane połowy działa tak samo), a~przy~remisach
			      na~$x$ rozstrzyga ograniczenie liczności, nie sama wartość~$x_l$.
			      Pierwsze rozwiązanie problemu w~czasie $\BO(N \log N)$ podali Shamos
			      i~Hoey (\textit{Closest-Point Problems}, 1975) inną metodą ---
			      przez diagram Woronoja.
		      \end{remark}
	...
	\begin{proof}[Dowód poprawności]
		...
		Przypuśćmy, że $p, q$ są parą punktów w najmniejszej odległości w $P$, $\delta' \coloneq d(p,q) < \delta$. 
    Niech $p = (x_p, y_p),\ q = (x_q, y_q)$. Bez straty ogólności załóżmy $y_p \leq y_q$. Pokażemy, że algorytm znajdzie
		...
		Ponadto $y_r$ musi być między $y_p$ i $y_q$: $y_p \leq y_r \leq y_q$, więc $(y_p - y_p \leq)\ y_r - y_p \leq y_q - y_p < \delta$
