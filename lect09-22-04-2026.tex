...
\begin{definition}
	\textbf{Matroid} to para $M = (S, \mathcal{A})$ taka, że:
	\begin{enumerate}
		...
		\item $\forall A, B \subseteq S \quad A \in \mathcal{A} \wedge B \subseteq A \implies B \in \mathcal{A}$,
		\item $\forall A, B \subseteq S \quad A, B \in \mathcal{A} \wedge |B| > |A| \implies \text{istnieje } x \in B \setminus A \text{ taki, że } A \cup \{x\} \in \mathcal{A}$ (własność wymiany).
	\end{enumerate}
...
\begin{proof}
	...
	Z własności 4. istnieje $x \in B \setminus A$ taki, że $A \cup \{x\} \in \mathcal{A}$ -- sprzeczność z maksymalnością $A$.
\end{proof}

\paragraph{Problem}
$M = (S, \mathcal{A})$ -- matroid, $w: S \to \Rp$ -- funkcja wagi.\\
Dla $A \subseteq S$ niech $w(A) = \sum_{x \in A} w(x)$ -- waga $A$.\\
\textbf{Problem:} Znaleźć zbiór niezależny o maksymalnej wadze w $M$.\\
$S = \{x_1, x_2, \ldots, x_n\}$.
...
\begin{proof}
	...
	$A \cup \{x_j\} \in \mathcal{A}$, więc z własności 3. $\{x_{i_1}, x_{i_2}, \ldots, x_{i_{l-1}}, x_j\} \subseteq A \cup \{x_j\}$ jest zbiorem niezależnym.\\
	...
	Z własności 3. $\{x_{i_1}, x_{i_2}, \ldots, x_{i_{t-1}}, y_s\} \subseteq A_{j-1} \cup \{y_s\}$ jest zbiorem niezależnym.\\
...
\textbf{Problem:} Znaleźć kolejność wykonania zadań minimalizującą sumę kar.

Załóżmy, że mamy pewien harmonogram. Zadanie $z_i$ nazywamy \underline{terminowym} w tym harmonogramie, jeśli jest zakończone do chwili $d_i$, w przeciwnym razie nazywamy je \underline{spóźnionym}.\\
