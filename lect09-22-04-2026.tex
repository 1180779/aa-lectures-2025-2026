	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[every node/.style={circle, draw, minimum size=0.6cm, inner sep=1pt}, baseline]
			...
			\begin{scope}
				...
				\node[draw=none] at (0.9, -0.6) {$F = \{e_1, e_2, e_4\} \in \mathcal{A}$};
			\end{scope}
			% prawa: cykl (zależny)
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[>=stealth, baseline]
			...
			\begin{scope}
				...
				\node at (1.0, -0.8) {$\{v_1, v_2\} \in \mathcal{A}$};
			\end{scope}
...
\begin{proof}
	...
	Przypuśćmy przeciwnie, że istnieje $x_j \in S \setminus A$ taki, że $A \cup \{x_j\} \in \mathcal{A}$. Wtedy $j < i_k$, bo w~przeciwnym wypadku algorytm dodałby $x_j$ do $A$ jako $(k+1)$-szy element. Niech $l$ będzie najmniejszą liczbą taką, że $j < i_l$; wówczas
	\[
		w(x_{i_1}) \geq \ldots \geq w(x_{i_{l-1}}) \geq w(x_j) \geq w(x_{i_l}).
	\]
	Ponieważ $A \cup \{x_j\} \in \mathcal{A}$, z~własności \ref{matroid:dziedziczność} zbiór $\{x_{i_1}, \ldots, x_{i_{l-1}}, x_j\} \subseteq A \cup \{x_j\}$ jest niezależny. Ale wtedy algorytm dodałby $x_j$ zamiast $x_{i_l}$ jako $l$-ty element --- sprzeczność. Zatem $A$ jest maksymalnym zbiorem niezależnym.

	\emph{Krok 2: $w(B) \leq w(A)$ dla każdego zbioru niezależnego $B$.}
	Rozważmy zbiór niezależny $B = \{y_1, y_2, \ldots, y_m\} \in \mathcal{A}$ z~$w(y_1) \geq w(y_2) \geq \ldots \geq w(y_m)$. Z~maksymalności $A$ (Lemat \ref{lemma:matroid:liczność}) mamy $m \leq k$. Pokażemy, że
	\[
		w(y_j) \leq w(x_{i_j}) \qquad \text{dla każdego } j \leq m.
	\]
	Przypuśćmy przeciwnie, że $w(y_j) > w(x_{i_j})$ dla pewnego $j$, i~rozważmy zbiory niezależne $A_{j-1} = \{x_{i_1}, \ldots, x_{i_{j-1}}\} \subseteq A$ oraz $B_j = \{y_1, \ldots, y_j\} \subseteq B$. Ponieważ $|B_j| = j > j-1 = |A_{j-1}|$, z~własności \ref{matroid:wymiana} istnieje $y_s \in B_j \setminus A_{j-1}$ taki, że $A_{j-1} \cup \{y_s\} \in \mathcal{A}$. Ponieważ $y_s \in B_j$, a~wagi $B$ są nierosnące, mamy $w(y_s) \geq w(y_j)$, skąd $w(y_s) \geq w(y_j) > w(x_{i_j})$; istnieje więc $t \leq j$ takie, że
	\[
