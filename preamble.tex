...
\definecolor{path}{RGB}{250, 0, 135} % ścieżka powiększająca (matching)
...
% Dowód obowiązkowy (inobow) MUSI iść bez własnego tcolorboxa: leży już w breakable
% obowiazkowybox, a tcolorbox nie obsługuje rzetelnie ZAGNIEŻDŻONYCH breakable pudełek
% (pierwszy objaw: pusty segment cream + treść uciekająca poza ramkę). Dlatego zamiast
% \tcolorboxenvironment (które opakowałoby KAŻDY proof) przepisujemy proof warunkowo:
%   inobow  → oryginalny proof, bez pudełka, na tle cream rodzica;
%   pomijalny → samodzielne szare pudełko (nie jest zagnieżdżone, więc breakable działa).
% AZ_SKIP wygrywa: nieobowiązkowe dowody znikają zupełnie (treść przez \BODY nie składana).
\ifboolexpr{bool {azskip} or bool {azhighlight}}{%
	\let\azoldproof\proof
	\let\azoldendproof\endproof
	\RenewEnviron{proof}[1][\proofname]{%
		\ifbool{inobow}%
			{\azoldproof[#1]\BODY\azoldendproof}%
			{\ifbool{azskip}%
				{}%                                          pomijalny + AZ_SKIP: pomiń
				{\begin{tcolorbox}[pomijalnystyle]\azoldproof[#1]\BODY\azoldendproof\end{tcolorbox}}}%
	}%
}{}
