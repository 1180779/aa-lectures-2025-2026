...
% Pełnoszerokościowe podświetlenie (do krawędzi strony) materiału obowiązkowego.
% Tło rozlewa się na marginesy (margin=2.5cm), a treść pozostaje w bloku tekstu.
% breakable: poprawnie łamie się między stronami.
\newtcolorbox{obowiazkowy}{%
  breakable, enhanced jigsaw,
  colback=obowiazkowybg,
  frame hidden, boxrule=0pt,
  arc=0pt, outer arc=0pt,
  grow to left by=2.5cm, grow to right by=2.5cm,
  left=2.5cm, right=2.5cm,
  top=8pt, bottom=8pt,
  boxsep=0pt,
  % Na stronach w całości objętych ramką tło dochodzi do górnej i dolnej
  % krawędzi kartki: w miejscach łamania strony dorysowujemy pas tła na
  % marginesie -- na dole strony, która się łamie, i na górze kontynuacji.
  % Pierwsza strona zaczyna się przy tekście, ostatnia kończy przy tekście.
  % (3cm > margines 2.5cm, więc pas pewnie dochodzi do krawędzi kartki.)
  % Margines górny/dolny zalewamy tłem tylko wtedy, gdy segment realnie sięga
  % krawędzi bloku tekstu (tj. strona jest w całości objęta ramką). tcolorbox
  % bywa, że krótką ramkę dzieli na „first” + pusty „last” -- taki krótki segment
  % traktujemy jak zakończenie (linia), a nie jak łamanie (zalanie marginesu).
  % \obyy = wysokość bieżącego segmentu (north - south).
  % Linia górna na początku, dolna na końcu; nasycony kolor = lepszy kontrast.
  % \obyy = wysokość segmentu (north-south); zawiera padding (\(\approx\)16pt),
  % więc „pusty ogon” ma ok. 0,5cm. Próg 0,9cm odrzuca takie segmenty-widma
  % (rysujemy nic). Próg 4cm = segment realnie wypełnia stronę (zalewamy margines).
  overlay unbroken={%
    \pgfpointdiff{\pgfpointanchor{frame}{south}}{\pgfpointanchor{frame}{north}}%
    \pgfgetlastxy{\obxx}{\obyy}%
    \ifdim\obyy>0.9cm\relax
      \fill[obowiazkowyline] (frame.north west) rectangle ([yshift=-1.5pt]frame.north east);
      \fill[obowiazkowyline] (frame.south west) rectangle ([yshift=1.5pt]frame.south east);
    \fi},
  overlay first={%
    \pgfpointdiff{\pgfpointanchor{frame}{south}}{\pgfpointanchor{frame}{north}}%
    \pgfgetlastxy{\obxx}{\obyy}%
    \ifdim\obyy>4cm\relax
      \fill[obowiazkowyline] (frame.north west) rectangle ([yshift=-1.5pt]frame.north east);
      \fill[obowiazkowybg] (frame.south west) rectangle ([yshift=-3cm]frame.south east);
    \else\ifdim\obyy>0.9cm\relax
      \fill[obowiazkowyline] (frame.north west) rectangle ([yshift=-1.5pt]frame.north east);
      \fill[obowiazkowyline] (frame.south west) rectangle ([yshift=1.5pt]frame.south east);
    \fi\fi},
  overlay middle={%
    ...
    \ifdim\obyy>0.9cm\relax
      \fill[obowiazkowybg] (frame.south west) rectangle ([yshift=-3cm]frame.south east);
      \fill[obowiazkowybg] (frame.north west) rectangle ([yshift=3cm]frame.north east);
    \fi},
  overlay last={%
    \pgfpointdiff{\pgfpointanchor{frame}{south}}{\pgfpointanchor{frame}{north}}%
    \pgfgetlastxy{\obxx}{\obyy}%
...
\AtEndEnvironment{obowiazkowy}{\par\removelastskip}
...
\BeforeBeginEnvironment{obowiazkowy}{\Needspace*{6\baselineskip}}
