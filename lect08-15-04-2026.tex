...
Z Lematu 1 wynika, że można zacząć konstrukcję optymalnego drzewa połączeniem dwóch symboli o najmniejszej częstości.
...
\begin{proof}
	...
	Przypuśćmy, że $T$ nie reprezentuje optymalnego kodu dla $(A, f)$. Niech $T''$ będzie drzewem reprezentującym kod prefiksowy dla $(A, f)$ takim, że $B(T'') < B(T)$. Z lematu 1 możemy wybrać $T''$ tak, aby $x$ i $y$ miały wspólnego rodzica i największą głębokość w $T''$. Niech $T'''$ będzie drzewem powstałym z $T''$ poprzez usunięcie liści $x$ i $y$, nadanie ich rodzicowi etykiety $z$ z kluczem $f(x) + f(y)$.
...
\begin{proof}
	...
	Mamy $|C'| = n - 1$. Z założenia indukcyjnego algorytm znajduje drzewo $T'$ reprezentujące optymalny kod prefiksowy dla $(C', f')$. Na mocy Lematu 2, zastąpienie liścia $z$ w $T'$ wierzchołkiem z dziećmi $x$ i $y$ (co właśnie robi algorytm Huffmana) daje drzewo $T$ będące optymalnym kodem prefiksowym dla $(C, f)$.
\end{proof}
